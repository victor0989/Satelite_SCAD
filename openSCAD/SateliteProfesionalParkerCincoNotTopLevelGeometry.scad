...
// Torus (aro) para parachoques de borde
module torus(R, r, $fn=96){
  rotate_extrude(angle=360, $fn=$fn)
    translate([R,0,0]) circle(r=r, $fn=max(24, ceil($fn/3)));
}

// Perfil parabólico (r vs z): r(z) = sqrt(4*f*z), con f = d^2 / (16*depth)
function parabola_f(d, depth) = (d*d) / (16*depth);

// Crea una "tira" de espesor t sobre el perfil parabólico y la rota 360º
module parabola_shell(d, depth, t=1.0, steps=72){
  f = parabola_f(d, depth);
  outer_pts = [for (i=[0:steps]) 
    let(z = i*depth/steps, r = sqrt(max(0, 4*f*z))) [r, z]
  ];
  inner_pts = [for (i=[steps:-1:0]) 
    let(z = i*depth/steps, r = max(0.01, sqrt(max(0,4*f*z)) - t)) [r, z]
  ];
  rotate_extrude($fn=steps*2)
    polygon(points=concat(outer_pts, inner_pts));
}

// Paraboloide multicapa: layers = [[t_mm, color, alpha], ...]
module dish_parabola_layered(d, depth, layers=[[1,"gray",0.8]], steps=72){
  t_cum = 0;
  for (idx=[0:len(layers)-1]) {
    t_i = layers[idx][0];
    col = layers[idx][1];
    a   = layers[idx][2];
    // Reducimos ligeramente el diámetro "efectivo" para no invadir capas exteriores.
    color(col, a)
      parabola_shell(d - 2*t_cum, depth, t=t_i, steps=steps);
    t_cum = t_cum + t_i;
  }
}
...
parker_probe();
// --- Llamada al ensamblado principal ---
rear_comms_and_boom();  // o cualquier módulo que quieras renderizar
